...
func setup(description: String, repeat: int, repeat_callable: Callable, error_predicate: Callable) -> void:
	...
	if _panel_stylebox == null:
		printerr("Could not get task checkbox panel stylebox. Task scene instance will not render properly.")
	else:
...
func _process(_delta: float) -> void:
	...
	status = Status.ERROR if error_predicate.call(self) else (Status.DONE if current_repeat == repeat else Status.NOT_DONE)
