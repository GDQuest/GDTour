...
func get_plural_message(key: StringName, src_message: StringName, src_plural_message: StringName, n: int, context: StringName = "") -> String:
	var result := ""
	if has_translation(key):
		result = translations[key][locale].get_plural_message(src_message, src_plural_message, n, context)
...
func get_tour_plural_message(src_message: StringName, src_plural_message: StringName, n: int, context: StringName = "") -> String:
...
func get_resource_path(path: String) -> String:
	var suffix := ":%s" % locale
	var filtered := (
		Array(translation_remaps.get(path, []))
		.filter(func(p: String) -> bool: return p.ends_with(suffix))
	)
