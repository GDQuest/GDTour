...
func _build() -> void:
	...
	bubble_add_text(
		[
			...
			"",
			gtr("Press [b]F8[/b] or close the game window to stop the game.")
		]
	)
	complete_step()
	...
	bubble_add_text(
		[
			gtr("The screen was completely gray because the scene currently lacks a camera."),
			"",
			gtr(
