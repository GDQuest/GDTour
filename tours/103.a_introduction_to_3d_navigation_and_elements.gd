...
func _build() -> void:
	...
	bubble_add_text([gtr("I just opened the scene unfinished_game.tscn for you."), "", gtr("This small game scene has a character, platforms, and a flag. Run the scene by clicking the \"play current scene\" button at the top right of the editor or pressing F6 on your keyboard."), "", gtr("You'll see a completely gray screen... Don't panic! It's what we expect. We'll explain why after you try!"), "", gtr("Press F8 or close the game window to stop the game.")])
	complete_step()
